...
class Person:
    ...
    def _get_lieu_residence(
            self):  # On n'accède pas, depuis l'extérieur de la classe, à une méthode commençant par un souligné _
    ...
    lieu_residence = property(_get_lieu_residence,
                              _set_lieu_residence)  # Une propriété peut être utilisé plus naturellement en dehors de la classe
...
class Duree:
    ...
    def __eq__(self,
               autre_duree):  # Opérateur d'égalité (equal). Renvoie True si self et objet_a_comparer sont égaux, False sinon.
...
class Temp:
    ...
    def __setstate__(self,
                     dict_attr):  # Modification du dictionnaire d'attribut après la serialisation, peut renvoyer autre chose que le dictionnaire
