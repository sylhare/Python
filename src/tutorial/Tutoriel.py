...
def table(nb, max=10):#max=10 permet de fournir une valeur par défaut, pas obligé de remplir le champs pour l'utilisateur
    i = 0
    if max<0:
        max=10
...
def controler_temps(nb_secs):
    ...
    def decorateur(fonction_a_executer):
        ...
        def fonction_modifiee(*parametres_non_nommes, **parametres_nommes):
            ...
            ret = fonction_a_executer(*parametres_non_nommes, **parametres_nommes)#Execute la fonction avec ses paramètres
            tps_apres = time.time()
